...
// console.log(int, typeof int); //22 number
// console.log(float, typeof float); //22.5 number
...
// Answer :-
// 1. Math.round() is a method of the Math object, while .toFixed() is a method of the Number object.
// 2. Math.round() returns an integer, while .toFixed() returns a string representation of the number with a specified number of decimal places.
// 3. Math.round() is used for rounding numbers to the nearest integer, while .toFixed() is used for formatting numbers to a specific number of decimal places.
...
// Answer :-
// let price = 99.99
// console.log(Math.floor(price), Math.ceil(price), Math.round(price), price.toFixed(1)) // 99 100 100 "100.0"
// price.toFixed(1) returns a string, while the other three methods return numbers.
...
// toFixed() does not change the original variable; it returns a new string representation of the number.
...
// Answer :-
// a) The smallest value it can ever print is 1.
// b) The largest value it can ever print is 10.
// c) Math.random() can never return exactly 1 because it generates a floating-point number in the range [0, 1), meaning it includes 0 but excludes 1.
...
// The + symbol concatenates the two string representations of the numbers, resulting in "10.6610.66".
...
// console.log(roundTo5(43));
// console.log(roundTo5(99));
// console.log(roundTo5(76));
...
//     let firstDigit = Math.floor(Math.random() * 4) + 6; // generates a number between 6 and 9
//     let remainingDigits = Math.floor(Math.random() * 1000000000); // generates a 9-digit number
...
// console.log(roundedAmount, typeof roundedAmount); // 1234.57 'number'
// console.log(amount.toFixed(2), typeof amount.toFixed(2)); // "1234.57" 'string'         
// The difference is that roundedAmount is a number, while amount.toFixed(2) returns a string representation of the number.
...
// Answer :-
// Math.floor(-4.2) = -5, because floor always rounds down to the next lowest integer.
// Math.ceil(-4.8) = -4, because ceil always rounds up to the next highest integer.
// Math.round(-4.5) = -4, because round rounds to the nearest integer, and -4.5 is exactly halfway between -4 and -5, so it rounds up to -4.
// Math.floor(-4.2) is not -4 because "rounding down" means moving away from zero, so it goes to -5 instead of -4.  
...
// Answer :-
// The output range of Math.random() is [0, 1), 
// meaning it can return a minimum value of 0 (inclusive) and a maximum value of 1 (exclusive). 
// It can never reach the maximum value of 1.
// Three real-world uses of Math.random() are: 
// 1. Generating random passwords or tokens
// 2. Simulating random events in games
// 3. Creating random data for testing purposes    
...
// The reason for this is that 0.1 and 0.2 cannot be represented exactly in binary floating-point format, leading to a small precision error when they are added together. 
// To fix the comparison, you can use the toFixed() method to round the result to a specific number of decimal places before comparing:
...
// } 
// console.log(generateAadhaarNumber());  
...
let randomPercent = Math.floor(Math.random() * (15 - 5 + 1)) + 5; // random between 5 and 15
